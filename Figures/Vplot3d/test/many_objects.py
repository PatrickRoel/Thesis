...
os.environ['CONF_PATH'] = str(Path.cwd() / 'Figures' / 'Vplot3d')

save_file_path = Path(__file__).resolve().parent 
os.chdir(save_file_path)
save_svg_tex('many_objects', fontsize=20, scour=True)
os.chdir(Path.cwd().parent)  # Optionally restore previous working directory
plt.close()
